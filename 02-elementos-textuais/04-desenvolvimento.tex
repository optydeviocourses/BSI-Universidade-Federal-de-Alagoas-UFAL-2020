...
Outra caracter\'{i}stica da transforma\c{c}\~{a}o ``fato\_denuncia.ktr'' é o uso do \textit{step Calculator}, como no 
...
os futuros campos da tabela de fato ``fato\_denuncia'' que será criada e receberá os dados conforme o processo de ETL configurado nesta transformação.
...
Depois da configura\c{c}\~{a}o do \textit{step Table output} - fato\_denuncia, clicando no botão ``SQL'' na tela do \textit{step}, 
da figura abaixo, na sequência aparecer\'{a} a tela do \textit{Simple SQL editor}, e ap\'{o}s, um clique no bot\~{a}o ``execute'' 
desta tela, a tela do \textit{Results of the SQL statements} apresenta o resultado da cria\c{c}\~{a}o f\í{i}sica da tabela 
...
Ao ser executada a transformação ``fato\_denuncia.ktr'', o resultado do 
processo aparecerá no quadro "Execution Results" da figura abaixo.

\begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Executando a transformação ``fato\_denuncia.ktr''}
...
O resultado final da transformação ``fato\_denuncia.ktr'', após ser executada é apresentada na Figura 85, 
abaixo, é percebermos os campos da tabela ``fato\_denuncia'', com os códigos de cada tabela de dimensão.
Assim, o objetivo desta transformações foi alcançado.

\begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Resultado da transformação e carga no Banco de Dados ``DW\_181'' na tabela fato\_denuncia}
...
Quando executamos o ``job\_denuncia.kjb'' \'{e} o mesmo não tem nenhum erro no fluxo das transformações, 
este job se apresentará conforme a figura logo abaixo:
...
Neste t\ó{o}pico finalizamos a fase de ETL usando o PDI, e seus recursos usando os componentes de \textit{Steps, Transformations e Jobs}.
...
Inicialmente executaremos o servidor \textit{Tomcat} conforme a figura abaixo, e para isso devemos usar o terminal em linha de comando do Sistema Operacional MacOSX Catalina, e nos direcionando via comando do console para a pasta:
