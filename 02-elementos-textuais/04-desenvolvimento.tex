...
begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Criando a tabela ``dim\_semana'' no banco de dados usando o step Table dim\_semana}
...
begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Criando a tabela ``dim\_trimestre" no banco de dados usando o step Table dim\_trimestre}
...
begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Criando a tabela ``dim\_semestre" no banco de dados usando o step Table dim\_semestre}
...
begin{figure}[H]
	\vspace*{0,2cm}
    \centering
    \caption{Criando a tabela ``dim\_ano'' no banco de dados usando o step Table dim\_ano}
...
perceber se o processo de transforma\c{c}\~{a}o e carga sai conforme planejado para todas as transforma\c{c}ões. 
...
nela iremos usar o step String cut, nesta n\~{a}o houve a necessidade do uso de componentes como o Calculator. 

O único detalhe \'{e} a movimenta\c{c}\~{a}o dos dados entre o step Sort rows para o Unique rows, principais, 
...
Assim, iremos produzir dentro do ``DW\_181'', duas novas dimensões para que possamos criar an\'{a}lises que podem usar estas dimensões.
...
Dividimos a transforma\c{c}ões em três caminhos, um caminho como sendo o principal que parte do 
...
a figura 72 logo abaixo, demonstra a configura\c{c}\~{a}o dos steps do caminho principais.
...
begin{figure}[H]
    ...
    \label{fig:ilustfigresdimanodim-minuto}
\end{figure}
\vspace*{-0,9cm}
{\raggedright \fonte{Autor desta monografia, 2020.}} \\

Tendo como base a configura\c{c}\~{a}o do \textit{step Table} dim\_hora, configuramos o step Table dim\_minuto, 
...
que gerou duas dimensões com os dados padronizados inseridos nelas.
